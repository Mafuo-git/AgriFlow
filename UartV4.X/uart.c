...
//    RCSTA1bits.ADDEN = SET;                     // address detect enable
    ...
    SPBRG1 = 16;   // set baud rate divider
...
void UARTWriteStrHex(char *data){
    ...
    while (data[i]){
    UARTWriteByte(hexa[data[i] / 16]);     // write ASCII value of hexadecimal high nibble
    UARTWriteByte(hexa[data[i++] % 16]);     // write ASCII value of hexadecimal low nibble
    }
    UARTWriteByte(0x0D);      // write Carriage Return
    UARTWriteByte(0x0A);      // write Line Feed (New Line)) 
}
